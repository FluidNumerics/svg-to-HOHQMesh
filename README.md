...
A simple script to generate [HOHQMesh](https://github.com/trixi-framework/HOHQMesh) `.control` files from SVGs. The High Order Hex-Quad Mesh (HOHQMesh) package is used to generate all-quadrilateral meshes with high order boundary information. The meshes generated by HOHQMesh can be used in [SELF](https://github.com/fluidnumerics/SELF) and in [Trixi.jl](https://github.com/trixi-framework/Trixi.jl)

## Licensing
svg-to-HOHQMesh is licensed for use under a [3 Clause BSD with attribution license](./LICENSE). [Fluid Numerics](https://www.fluidnumerics.com) is a small family-owned business. We want to make svg-to-HOHQMesh available to anyone who wants to build meshes from vector graphics. Under the license, you can use, modify, and redistribute svg-to-HOHQMesh so long as attribution is given to Fluid Numerics. 
...
### Overview
The [Spectral Element Library in Fortran](https://github.com/fluidnumerics/self) is a OO Fortran library that can be used for easily building conservation law solvers. SELF comes equipped with the capability to run on CPU and GPU (both AMD and Nvidia) platforms. Large problems are easily tackled with domain decomposition and built in methodology leveraging MPI. When constructing a simulation using SELF, all solvers require a mesh that described the physical geometry of the problem. Currently, SELF permits either structured mesh construction using in-built methods or reading in mesh files output by [the High Order Preprocessor (HOPr)](https://github.com/hopr-framework/hopr). HOPr is capable of converting 2-D HOHQMesh mesh files into a standardized format the SELF is equipped to read in.

To create a 2-D mesh for SELF from an Inkscape SVG, the workflow is as follows

* Draw your boundaries in Inkscape
* Convert your SVG to a HOHQMesh control file using `python svg-to-control.py <your-file>.svg`
* Create a HOHQMesh mesh with `hohqmesh`
* Convert your HOHQMesh mesh file to a HOPr file using HOPr.
